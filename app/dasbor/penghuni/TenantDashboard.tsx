...
import { useApp, useUserBookings, useNotifications, useTenantSurveys, useIsLoggedIn, useTrustedContact, useTrustedContactNotifications } from "@/lib/app-context"
...
import {
  ...
  CalendarSearch, MapPin, Heart, Mail, Phone, Users, Edit, Trash2, Save,
} from "lucide-react"
...
export default function TenantDashboard() {
  ...
  const userPayments = state.currentUser ? state.payments.filter((p) => p.tenantId === state.currentUser.id) : []
  const userServices = state.currentUser ? state.serviceRequests.filter((sr) => sr.tenantId === state.currentUser.id) : []
  ...
  const [tcRelationship, setTcRelationship] = useState(trustedContact?.relationship ?? "")
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <DashboardShell>
        {/* Summary / Ringkasan */}
        {(tab === "ringkasan" || !["booking", "pembayaran", "layanan", "notifikasi", "survey", "kontak"].includes(tab)) && (
              ...
              {trustedContact ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-card-foreground">{trustedContact.name}</p>
                      <p className="text-sm text-muted-foreground">{trustedContact.relationship}</p>
                    </div>
                  </div>
                  <button
                ...
                <div className="text-center py-4">
                  <p className="text-sm text-muted-foreground mb-3">Belum ada kontak terpercaya</p>
                  <button
                    onClick={() => router.push("?tab=kontak")}
                    ...
                    <div key={b.id} className="rounded-xl border border-border bg-card p-6 transition-all hover:shadow-sm">
                      ...
                      {b.trustedContactId && (
                        <div className="mt-4 flex items-center gap-2 rounded-lg bg-pink-50 px-3 py-2 dark:bg-pink-900/20">
                          ...
                          <span className="text-sm text-pink-700 dark:text-pink-300">Kontak terpercaya akan dinotifikasi</span>
                        </div>
                      )}
                    </div>
                  )
        ...
        {tab === "kontak" && (
          <div className="flex flex-col gap-6">
            <div>
              <h1 className="text-2xl font-bold text-foreground">Kontak Terpercaya</h1>
              <p className="text-muted-foreground">Kelola kontak orang tua/wali untuk menerima notifikasi</p>
            </div>
              ...
              {trustedContact && !editingContact ? (
                <div>
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-lg font-medium text-card-foreground">{trustedContact.name}</p>
                        <p className="text-sm text-muted-foreground">{trustedContact.relationship}</p>
                      </div>
                    </div>
                  ...
                  {trustedContactNotifs.length > 0 && (
                    <div className="mt-6 border-t border-border pt-4">
                      <h3 className="font-medium text-card-foreground mb-3">Riwayat Notifikasi Terkirim</h3>
                      <div className="flex flex-col gap-2">
                        {trustedContactNotifs.map((tcn) => (
                          <div key={tcn.id} className="rounded-lg bg-secondary/50 p-3 text-sm">
                            <div className="flex items-center justify-between">
                              <span className="font-medium text-card-foreground capitalize">
                                {tcn.type.replace(/_/g, " ")}
                              </span>
                              <span className="text-xs text-muted-foreground">{tcn.sentAt}</span>
                            </div>
                            <p className="text-muted-foreground mt-1">{tcn.message}</p>
                            <div className="flex items-center gap-1 mt-2 text-xs text-muted-foreground">
                              {tcn.method === "email" ? <Mail className="h-3 w-3" /> : <Phone className="h-3 w-3" />}
                ...
                <div className="flex flex-col gap-4">
                  <h3 className="font-semibold text-card-foreground">
                    {trustedContact ? "Edit Kontak Terpercaya" : "Tambah Kontak Terpercaya"}
                  </h3>
                  <div>
                  ...
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-foreground">Hubungan</label>
                    <select
